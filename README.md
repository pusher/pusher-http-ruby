...
Triggering an event will return a hash of channels to event IDs as generated by Pusher's backend:

``` ruby
Pusher.trigger(['channel_1', 'channel_2'], 'event_name', {foo: 'bar'})

=> { event_ids: { "channel_1": "sdfsf", "channel_2": "sdfgsfg" } }
```

### Event Buffer

Version 1.0.0 of the library introduced support for event buffering. The purpose of this functionality is
...
The event ID values are accessed via the hash that is returned from the `trigger` call.

```ruby
# Trigger on single channel
result = Pusher.trigger('channel_1', 'event_name', {foo: 'bar'})
event_id = result[:event_ids]['channel_1']

# Trigger on multiple channels
result = Pusher.trigger(['channel_1', 'channel_2', 'channel_3'], 'event_name', {foo: 'bar'})
event_ids = result[:event_ids]
channel_1_event_id = event_ids['channel_1']
channel_2_event_id = event_ids['channel_2']
channel_3_event_id = event_ids['channel_3']
```
